                    ...
                    <div className={selected === 4 ? "flex items-center w-full justify-center" : "grid grid-cols-2 tablet:grid-cols-1 place-items-center gap-4 mb-4"}>
                        {selected === 1 ? (
                            ...
                            wiki.wiki.filter((wiki1) => wiki1.name.toLowerCase().includes(query.toLowerCase())).map((wiki1, index) => (
                                ...
                                <Link key={index} to={wiki1.link} className="w-[300px] tablet:w-[47vw] h-[150px] hover:shadow-blueshadow hover:shadow-2xl transition-all duration-150 border-4 border-blueborder flex items-center justify-center">
                                    <div className="m-2 h-full w-full flex border-2 border-t-0 border-b-0 border-blueborder">
                                        <div className="bg-bluebg border-b-0 border-l-0 border-blueborder text-bluetext border-2 h-[22px] absolute mt-[120px] text-center font-bold px-1">{wiki1.name} - {wiki1.addon}</div>
